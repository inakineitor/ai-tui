# Plan: Fix JSX Namespace Error in @packages/core

## Problem

TypeScript error at `src/context/theme/index.tsx:633`:
```
error TS2503: Cannot find namespace 'JSX'.
```

The `jsxImportSource` is set to `@opentui/react`, which means JSX types should come from there, not from `react`. The `jsxImportSource` setting handles JSX syntax transformation, but doesn't automatically make `JSX` available as a namespace for explicit type annotations.

## Solution

Import `JSX` from `@opentui/react/jsx-runtime` (the jsxImportSource).

Note: In `@opentui/react`, `JSX.Element = React.ReactNode`, so we can also just remove the explicit return type and let TypeScript infer it.

### Option A: Import JSX from jsxImportSource (Preferred - explicit)

```diff
+ import type { JSX } from "@opentui/react/jsx-runtime";
```

### Option B: Remove explicit return type (Simpler)
...
Run `pnpm run check-types` in `packages/core` - should pass with no errors.

---

# Previous Plan: Replace Bun in @packages/core
...
**Current Bun APIs → Node.js Equivalents:**
