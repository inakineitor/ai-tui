...
// ColorGenerator type - inlined to avoid type resolution issues with opentui-spinner
type ColorGenerator = (
  frameIndex: number,
  charIndex: number,
  totalFrames: number,
...
type CalculateColorIndexParams = {
  frameIndex: number;
  charIndex: number;
  totalChars: number;
  options: Pick<
...
function calculateColorIndex(params: CalculateColorIndexParams): number {
  const { frameIndex, charIndex, totalChars, options, state } = params;
...
function createKnightRiderTrail(
  ...
  let cachedState: ScannerState | undefined;
  ...
  return (
    ...
    if (frameIndex !== cachedFrameIndex || cachedState === undefined) {
...
export type KnightRiderOptions = {
  width?: number;
...
export function createFrames(options: KnightRiderOptions = {}): string[] {
  ...
  const frames = Array.from({ length: totalFrames }, (_f, frameIndex) => {
    return Array.from({ length: width }, (_c, charIndex) => {
      const index = calculateColorIndex({
        frameIndex,
        charIndex,
        totalChars: width,
